...
http.listen(port, function () {
    ...
    mongoClient.connect("mongodb://localhost:27017", async function (error, client) {
        ...
        const db = client.db("technostore-ar")
        global.db = db
        ...
        app.post("/paidViaPaypal", async function (request, result) {
            const productsCart = JSON.parse(request.fields.products)
            const details = JSON.parse(request.fields.details)
            const name = request.fields.name
            const email = request.fields.email
            const mobile = request.fields.mobile
            const country = request.fields.country
        ...
        app.post("/confirmStripePayment", async function (request, result) {
            ...
            const stripe = await global.getStripeObj(result)
            if (stripe == null) {
                return
            }
        ...
        app.post("/getStripeClientSecret", async function (request, result) {
            const total = request.fields.total * 100
